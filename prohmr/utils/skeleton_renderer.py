...
class SkeletonRenderer:
    ...
    def __call__(self,
        ...
        gt_keypoints_3d[:, :, :-1] = gt_keypoints_3d[:, :, :-1] - gt_keypoints_3d[:, [25+14], :-1] + pred_keypoints_3d[:, 0, [25+14]]
        ...
        if camera_translation is None:
            camera_translation = torch.tensor([0.0, 0.0, 2 * self.cfg.EXTRA.FOCAL_LENGTH / (0.8 * self.cfg.MODEL.IMAGE_SIZE)]).unsqueeze(0).unsqueeze(0).repeat(batch_size, num_samples, 1)
        else:
        ...
        gt_keypoints_3d_proj = perspective_projection(gt_keypoints_3d[:, :, :-1], rotation=rotation.repeat(batch_size, 1, 1), translation=camera_translation[:, 0, :], focal_length=focal_length.repeat(batch_size, 1), camera_center=camera_center.repeat(batch_size, 1))
        pred_keypoints_3d_proj = perspective_projection(pred_keypoints_3d.reshape(batch_size*num_samples, -1, 3), rotation=rotation.repeat(batch_size*num_samples, 1, 1), translation=camera_translation.reshape(batch_size*num_samples, -1), focal_length=focal_length.repeat(batch_size*num_samples, 1), camera_center=camera_center.repeat(batch_size*num_samples, 1)).reshape(batch_size, num_samples, -1, 2)
        gt_keypoints_3d_proj = torch.cat([gt_keypoints_3d_proj, gt_keypoints_3d[:, :, [-1]]], dim=-1).cpu().numpy()
        pred_keypoints_3d_proj = torch.cat([pred_keypoints_3d_proj, keypoints_to_render.reshape(batch_size, 1, -1, 1).repeat(1, num_samples, 1, 1)], dim=-1).cpu().numpy()
        rows = []
        ...
        pred_keypoints_3d_side = torch.einsum('bmni,ij->bmnj', pred_keypoints_3d_side, R)
        gt_keypoints_3d_proj_side = perspective_projection(gt_keypoints_3d_side[:, :, :-1], rotation=rotation.repeat(batch_size, 1, 1), translation=camera_translation[:, 0, :], focal_length=focal_length.repeat(batch_size, 1), camera_center=camera_center.repeat(batch_size, 1))
        pred_keypoints_3d_proj_side = perspective_projection(pred_keypoints_3d_side.reshape(batch_size*num_samples, -1, 3), rotation=rotation.repeat(batch_size*num_samples, 1, 1), translation=camera_translation.reshape(batch_size*num_samples, -1), focal_length=focal_length.repeat(batch_size*num_samples, 1), camera_center=camera_center.repeat(batch_size*num_samples, 1)).reshape(batch_size, num_samples, -1, 2)
        gt_keypoints_3d_proj_side = torch.cat([gt_keypoints_3d_proj_side, gt_keypoints_3d_side[:, :, [-1]]], dim=-1).cpu().numpy()
        pred_keypoints_3d_proj_side = torch.cat([pred_keypoints_3d_proj_side, keypoints_to_render.reshape(batch_size, 1, -1, 1).repeat(1, num_samples, 1, 1)], dim=-1).cpu().numpy()
        for i in range(batch_size):
            ...
            for j in range(num_samples):
                ...
                for op, gt in zip(openpose_indices, gt_indices):
                    if pred_keypoints_3d_proj[i, j, gt, -1] >= body_keypoints_3d_proj[op, -1]:
            ...
            for j in range(num_samples):
                ...
                for op, gt in zip(openpose_indices, gt_indices):
                    if pred_keypoints_3d_proj_side[i, j, gt, -1] >= body_keypoints_3d_proj[op, -1]:
                        body_keypoints_3d_proj[op] = pred_keypoints_3d_proj_side[i, j, gt]
